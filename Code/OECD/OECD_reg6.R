# Script: OECD_reg6.R
# Purpose: Regression 6 on OECD Episode Classification
# Input: Data/Clean Data/full_OECD_dataset.csv
# Output: Output/OECD_reg6.tex
...
episode_df <- df %>%
  ...
  summarise(
    # Dependent variable: deceleration in pace of debt accumulation
  ...
  filter(!is.na(decel_debt_ep))

# 5 - Specifications
# Same set as regression 5
...
run_debt_ols <- function(data, include_rer = FALSE, money_var = NULL, comp_var = NULL) {
  
  vars_keep <- c(
    "country_code", "decel_debt_ep", "NEED_ADJ_pre", "GAP_pre",
  ...
  model_formula <- as.formula(
    paste("decel_debt_ep ~", paste(rhs_vars, collapse = " + "))
  )
...
latex_lines <- c(
  "\\begin{table}[!htbp]",
  "\\centering",
  "\\caption{Deceleration in the pace of debt accumulation (OECD Episode Identification)}",
...
writeLines(latex_lines, "Output/OECD_reg6.tex")