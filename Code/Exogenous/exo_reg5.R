# Script: exo_reg5.R
# Purpose: Regression 5 on Exogenous Episode Classification
# Input: Data/Clean Data/full_gen.csv
# Output: Output/exo_reg5.tex
...
episode_df <- df %>%
  ...
  summarise(
    # Dependent variable: cumulated adjustment over the whole episode
  ...
  filter(
    !is.na(cum_adjustment),
    cum_adjustment >= 1
  )

# 5 - Specifications
# 1: need adj + output gap + rer
# 2-6: need adj + output gap + one monetary variable
# 7: need adj + output gap + share of expenditure cuts
# 8: need adj + output gap + share of capital-expenditure cuts
# 9-13: need adj + output gap + share of expenditure cuts + one monetary variable
...
run_cum_trunc <- function(data, include_rer = FALSE, money_var = NULL, comp_var = NULL) {
  
  vars_keep <- c(
    "country_code", "cum_adjustment", "NEED_ADJ_pre", "GAP_pre",
  ...
  model_formula <- as.formula(
    paste("cum_adjustment ~", paste(rhs_vars, collapse = " + "))
  )
  
  # Lower truncation at 1, consistent with the episode definition
  model <- truncreg(
...
latex_lines <- c(
  "\\begin{table}[!htbp]",
  "\\centering",
  "\\caption{Size of cumulated adjustment during episode (Exogenous Identification)}",
  "\\label{tab:reg5_cumadjust}",
  "\\begin{threeparttable}",
  "\\scriptsize",
  "\\setlength{\\tabcolsep}{2.2pt}",
  "\\renewcommand{\\arraystretch}{1.05}",
  "\\begin{tabular}{@{}>{\\raggedright\\arraybackslash}p{0.34\\linewidth}*{13}{>{\\centering\\arraybackslash}p{0.046\\linewidth}}@{}}",
  "\\toprule",
...
writeLines(latex_lines, "Output/exo_reg5.tex")